...
class ConsumptionAnalyticsRepository(IConsumptionAnalyticsRepository):
    ...
    async def get_daily_totals(
        ...
        try:
            ...
            all_meals_result = await self.db.execute(
            ...
            all_meals = all_meals_result.unique().scalars().all()
    ...
    async def get_today_summary(
        ...
        try:
            ...
            user_result = await self.db.execute(
                select(User)
                .options(joinedload(User.profile), joinedload(User.goal))
    ...
    async def get_weekly_summary(self, user_id: int, week_start_date: date) -> Dict:
        try:
    ...
    async def get_consumption_trends(self, user_id: int, days: int) -> Dict:
        try:
    ...
    async def get_calorie_distribution(self, user_id: int, days: int) -> Dict:
        try:
            end_date = datetime.utcnow().date()
            start_date = end_date - timedelta(days=days)
    ...
    async def get_meal_completion_stats(self, user_id: int, days: int) -> Dict:
        try:
            end_date = datetime.utcnow().date()
            start_date = end_date - timedelta(days=days)
            start_datetime = datetime.combine(start_date, datetime.min.time())
            ...
            all_meals = result.unique().scalars().all()
            ...
            for meal in all_meals:
                ...
                if meal_type not in stats_by_type:
                    stats_by_type[meal_type] = {"planned": 0, "consumed": 0, "skipped": 0}
    ...
    async def get_meal_calorie_distribution(self, user_id: int, days: int) -> Dict:
        try:
            end_date = datetime.utcnow().date()
            start_date = end_date - timedelta(days=days)
            start_datetime = datetime.combine(start_date, datetime.min.time())
    ...
    async def get_average_meal_timing(self, user_id: int, days: int) -> Dict:
        try:
            end_date = datetime.utcnow().date()
            start_date = end_date - timedelta(days=days)
            start_datetime = datetime.combine(start_date, datetime.min.time())
    ...
    async def get_adherence_by_day_of_week(self, user_id: int, weeks: int) -> Dict:
        try:
            end_date = datetime.utcnow().date()
            ...
            for day, stats in stats_by_day.items():
                ...
                result_dict[day] = round(adherence, 3)
    ...
    async def get_adherence_trend(self, user_id: int, days: int, bucket_size: int = 7) -> List[Dict]:
        try:
            end_date = datetime.utcnow().date()
            start_date = end_date - timedelta(days=days)
    ...
    # =========================================================================
    # RECIPE AND PREFERENCE ANALYTICS (Placeholders)
    # =========================================================================

    async def get_most_consumed_recipes(self, user_id: int, days: int, limit: int = 10) -> List[Dict]:
        return []

    async def get_least_consumed_recipes(self, user_id: int, days: int, limit: int = 10) -> List[Dict]:
        return []

    async def get_portion_size_preferences(self, user_id: int, days: int) -> Dict:
        return {"overall": {"average_multiplier": 1.0}}

    async def get_macro_balance_analysis(self, user_id: int, days: int) -> Dict:
        return {}

    async def get_calorie_macro_correlation(self, user_id: int, days: int) -> Dict:
        return {}
    ...
    async def get_current_adherence_streak(self, user_id: int) -> Dict:
        return {"current_streak_days": 0}

    async def get_milestone_progress(self, user_id: int) -> Dict:
